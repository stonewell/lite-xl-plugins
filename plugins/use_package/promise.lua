...
function Promise:resolve(result) self.result = result self.success = true for _, v in ipairs(self._done) do v(result) end return self end
function Promise:reject(result) self.result = result self.success = false for _, v in ipairs(self._fail) do v(result) end return self end
function Promise:forward(p) self:done(function(d) p:resolve(d) end):fail(function(d) p:reject(d) end) return self end
